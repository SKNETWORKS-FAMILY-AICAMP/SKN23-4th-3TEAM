...
- [4. 수집된 데이터 및 데이터 전처리](#4-수집된-데이터-및-데이터-전처리)
- [5. 4차 버전 신규/개선 기능](#5-4차-버전-신규개선-기능)
...
- 전년 대비 성장률: **약 21.8%**
- 외국인 오프라인 매출 비중: **약 28%**
- 업계에서는 **6조 원 돌파 기대감**이 언급될 정도로 성장세가 이어지고 있음

### 2.4. N년 후 예상 시장 규모

최근 성장 흐름이 유지된다고 가정하면, 올리브영 중심 K-뷰티 플랫폼 시장은 향후 3년 내 **10조 원대 규모**로 확대될 가능성이 있습니다.

온유는 이처럼 빠르게 성장하는 뷰티 커머스 환경에서 피부 분석, 성분 해석, 제품 추천, 퍼스널컬러, 피부 MBTI를 연결해 **개인화된 뷰티 경험**을 제공하는 AI 서비스로 기획되었습니다.

### 2.5. 기대효과
- 피부 분석, 성분 해석, 퍼스널컬러, 피부 MBTI를 하나의 흐름으로 통합
- 단순 추천을 넘어 사용자의 재방문과 공유를 유도하는 서비스 구조 설계
- 올리브영 기반 실사용 제품 추천으로 구매 연결 가능성 강화
- 웹뿐 아니라 앱 확장 구조까지 고려한 서비스로 발전
- AI 기능 시연에서 끝나지 않고 실제 사용자 경험과 운영 요소까지 반영한 뷰티 AI 플랫폼으로 고도화
- 
---
...
#### 한계점
- 피부 정량 분석 기능은 부족
- 성분 OCR, 피부 MBTI, 상담형 챗봇 등 통합 경험은 약함
- 스킨케어 전반보다 퍼스널컬러 중심 서비스 성격이 강함
...
- AI 피부 진단
- 화장품 매칭
- 사용자 리뷰 / 유튜버 리뷰 기반 제품 탐색

#### 강점
- 피부 진단과 뷰티 콘텐츠 소비를 함께 제공
- 사용자 친화적인 앱 경험이 축적됨
- 제품 탐색과 후기 소비 흐름이 자연스러움

#### 한계점
- 날짜별 피부 분석 비교 같은 리포트형 기능은 제한적
- 성분 OCR 분석, 피부 MBTI, 챗봇 기반 심화 상담은 상대적으로 약함
- 결과 공유 구조와 서비스 통합성은 온유가 더 강점이 있음
...
| 피부 MBTI | O | X | X |
| 챗봇 기반 피부 상담 | O | X | 제한적 |
| 날짜별 피부 분석 비교 | O | X | 제한적 |
| 웹캠 기반 퍼스널컬러 | O | O | O |
| 공유형 결과 링크 | O | X | 제한적 |
| 올리브영 기반 추천 흐름 | O | X | 제한적 |
...
### 5.3 사용자 튜토리얼 / 꿀팁 온보딩
- 첫 회원가입 후 튜토리얼 모달 제공
- 피부 분석, 피부 MBTI, 위시리스트, 퍼스널컬러 등 핵심 기능 소개
- 사용자 초기 적응 보조

### 5.4 챗봇 페르소나 스트리밍
- 사용자가 질문 후 답변을 기다리는 동안 페르소나 메시지를 표시
- 단순 로딩 문구를 UX 요소로 전환
- SSE 스트리밍 기반으로 대기 시간의 지루함 완화(10초마다 자동으로 항목 전환)

### 5.4.1 페르소나

- 관련 문서  
  - [페르소나 기획 노션](https://www.notion.so/321614937e7a80799db9d355aed64753?source=copy_link)

#### 정의
- 온유의 **옹글이**는 사용자의 피부 고민을 다정하게 들어주고, 현재 피부 상태를 이해하기 쉽게 설명하며, 성분과 화장품 정보를 친절하게 큐레이션해주는 **피부 코치형 · 성분 큐레이터형 챗봇**입니다.  
밝고 발랄한 에너지를 지니고 있지만 가볍게만 반응하지 않고, 사용자의 고민에 공감하며 부담 없이 피부 관리를 이어갈 수 있도록 돕습니다.  
또한 피부 상태를 단정적으로 판단하기보다 **“이럴 수 있어요”**, **“이 방향으로 관리해보면 좋아요”**와 같은 부드럽고 긍정적인 표현을 사용합니다.
...
### 5.6 OCR 방식 변경
- 기존 OCR 방식에서 VLM 기반 성분 추출 흐름으로 고도화
- 곡면 라벨, 작은 글씨, 맥락 기반 구분 성능 개선
...
### 5.10 추천 개수 제어
- “2개 추천해줘”, “3개만 보여줘” 같은 요청 반영
- 자연어 기반 추천 개수 제어
  
### 5.11 백엔드 엔드포인트 구조 정리
- API 경로 체계 정리
- 프론트-백 연결 구조 보완
- 일부 엔드포인트 전환 대응
...
### 5.15 Flutter 앱
- 주요 웹 기능을 앱에서도 활용할 수 있도록 Flutter 기반 앱 구현
