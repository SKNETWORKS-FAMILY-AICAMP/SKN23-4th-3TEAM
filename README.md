...
- [4. 4차 버전 신규/개선 기능](#4-4차-버전-신규개선-기능)
...
| 분류 | 의미 |
| --- | --- |
|  서비스 확장 | 기존 피부 분석/챗봇 중심 서비스에 새로운 사용자 기능 추가 |
|  개인화 강화 | 사용자별 맞춤 경험 및 결과 활용성 향상 |
|  사용자 경험 개선 | 회원가입부터 결과 확인까지 사용 흐름 개선 |
|  계정/운영 기능 강화 | 마이페이지 및 운영 관리 기능 보완 |

---

## 4.2 사용자 경험 및 서비스 확장 기능 — 🧩 서비스 확장

기존 3차 버전은 피부 분석, 성분 분석, 제품 추천 챗봇 중심의 구조였다면, 4차 버전에서는 피부 MBTI, 분석 이력, 온보딩, 정책 페이지, 마이페이지 기능까지 확장해 서비스 완성도를 높였습니다. 기존 문서상 앞단 기능은 피부 MBTI, 회원가입 UX, 마이페이지 기능 강화로 정리되어 있었습니다. 

### 4.2.1 피부 MBTI 및 분석 이력 — 🎯 개인화 강화

**Before:** 피부 분석 결과는 단발성 확인에 가까웠고, MBTI형 피부 성향 기능은 제공되지 않음  
**After:** 피부 MBTI 테스트와 결과 페이지를 추가하고, 피부 분석 결과를 날짜별로 조회·비교할 수 있도록 확장

| 항목 | 내용 |
| --- | --- |
| MBTI 기능 | 질문형 테스트 기반 피부 MBTI 결과 도출 |
| 결과 활용 | MBTI 결과 페이지 제공 + 챗봇 연동 |
| 분석 이력 | 날짜별 분석 결과 조회 |
| 비교 기능 | 특정 날짜 결과 비교 가능 |

**고도화 근거:** 단발성 분석 경험을 넘어서, 사용자 성향과 분석 이력을 함께 활용하는 개인화 서비스로 확장

---

### 4.2.2 회원가입 및 초기 사용자 경험 개선 — 🙋 사용자 경험 개선

**Before:** 회원가입 후 서비스 구조를 바로 이해하기 어려움, 약관 동의 및 안내 구조 미흡  
**After:** 회원가입 시 약관 동의 검증을 추가하고, 첫 가입 사용자 대상 온보딩 및 안내 페이지를 확장

| 항목 | 내용 |
| --- | --- |
| 약관 동의 | 이용약관 / 개인정보처리방침 동의 체크 및 필수 검증 |
| 온보딩 | 첫 회원가입 후 튜토리얼 모달 제공 |
| 안내 내용 | 피부 분석, 피부 MBTI, 위시리스트, 퍼스널컬러 등 핵심 기능 소개 |
| 정책 페이지 | 이용약관, 개인정보처리방침, FAQ, COPYRIGHT 페이지 추가 |
| 공통 UI | 푸터 적용 |

**고도화 근거:** 신규 사용자가 서비스를 빠르게 이해하고 진입 장벽 없이 사용할 수 있도록 초기 UX 보완

---

### 4.2.3 마이페이지 및 계정 관리 기능 강화 — 🛠 계정/운영 기능 강화

**Before:** 계정 관리와 사용자 문의 기능이 제한적이었고, 운영자 기능 분리도 부족  
**After:** QnA, 회원탈퇴, 닉네임 관리 기능을 추가해 사용자/관리자 관점의 마이페이지를 강화

| 항목 | 내용 |
| --- | --- |
| QnA | 사용자 문의 등록 / 조회 기능 추가 |
| 관리자 기능 | 전체 문의 관리 기능 추가 |
| 권한 분리 | 사용자 / 관리자 권한에 따른 기능 분리 |
| 회원탈퇴 | 마이페이지에서 직접 탈퇴 가능 |
| 닉네임 기능 | 닉네임 중복 확인 + 랜덤 닉네임 생성 |

**고도화 근거:** 단순 정보 조회 페이지가 아니라, 계정 관리와 운영 기능까지 수행하는 마이페이지로 확장

---

## 4.3. 챗봇 페르소나 — 🎯 개인화 강화

기존 문서에서는 온유의 챗봇 **옹글이**를 피부 코치형 · 성분 큐레이터형 챗봇으로 정의하고, 발랄함·공감성·긍정성·설명력을 핵심 성격으로 정리하고 있습니다. :contentReference[oaicite:3]{index=3}

**Before:** 피부/성분 정보를 전달하는 기능 중심 챗봇  
**After:** 사용자의 고민에 공감하고, 피부 상태와 성분 정보를 부드럽고 이해하기 쉽게 설명하는 페르소나형 챗봇
...
| 표현 방식 | “이럴 수 있어요”, “이 방향으로 관리해보면 좋아요”와 같은 부드럽고 긍정적인 표현 |
| 핵심 성격 | 발랄함, 친절함, 공감성, 긍정성, 다정함, 쉬운 설명력 |

**페르소나 기획 문서:**  
- [페르소나 기획 노션](https://www.notion.so/321614937e7a80799db9d355aed64753?source=copy_link)

**고도화 근거:** 단순 질의응답형 챗봇에서 벗어나, 사용자가 피부 고민을 부담 없이 상담할 수 있는 서비스형 캐릭터 경험 제공

### 4.4 챗봇 고도화

## 4.4.1 이용약관 및 개인정보처리방침
...
| # | 고도화 항목 | 분류 | Before (3차) | After (4차) |
| --- | --- | --- | --- | --- |
| 1 | 퍼스널컬러 분석 | 기능 확장 | 피부 분석 + 성분 분석만 | + 퍼스널컬러 14타입 판정 추가 |
| 2 | OCR 모델 전환 | 인프라 개선 | Qwen2.5-VL (GPU 필수) | GPT-4o-mini Vision (CPU 가능) |
| 3 | 웹캠 얼굴 감지 | 입력 품질 | 아무 사진 업로드 가능 | 실시간 3단계 검증 + 자동 촬영 |
| 4 | MBTI 챗봇 연동 | 개인화 | MBTI는 결과 페이지에서만 확인 | MBTI 데이터 → LLM 프롬프트 자동 주입 |
| 5 | 분석 실패 안내 통일 | UX 개선 | "잠시 오류가 발생했어요" | 4개 분석별 맞춤 가이드 + 촬영 팁 |
| 6 | 단계별 진행 표시 | UX 개선 | 단일 로딩 메시지 | 파이프라인 단계별 SSE 실시간 업데이트 |
| 7 | 피부 상식 모달 | UX 개선 | 빈 화면 대기 (13~27초) | 100개 상식 캐러셀 모달 (랜덤 20개) |
| 8 | 컬러 팔레트 HEX 파싱 | UX 개선 | 매핑 누락 시 팔레트 미생성 | 이중 방어 (프롬프트 HEX 강제 + 폴백) |

---

#### 4.4.2. 퍼스널컬러 분석

정면 얼굴 사진 1장을 GPT-4o Vision에 전달하여 피부톤, 눈동자 색, 입술 색감을 종합 분석하고, **14개 퍼스널컬러 타입** 중 가장 적합한 타입을 판정합니다.
...
| 브릿지 조합 | 감성 이름 |
| --- | --- |
| 봄 라이트 + 봄 브라이트 | 살구 버블 웜 |
| 여름 라이트 + 여름 뮤트 | 안개 이슬 쿨 |
| 가을 뮤트 + 가을 딥 | 메이플 포그 웜 |
| 겨울 브라이트 + 겨울 딥 | 별빛 베리 쿨 |
| 봄 라이트 + 여름 라이트 | 코튼 피치 뉴트럴 |
| 여름 뮤트 + 가을 뮤트 | 로즈 티 포그 뉴트럴 |
...
정면 얼굴 사진
    ↓
[1] GPT-4o Vision — 얼굴 검증 (사람인지 확인)
    ↓ 실패 시 → 맞춤 안내 메시지 반환
[2] GPT-4o Vision — SCORING_PROMPT
    - 피부 언더톤, 밝기, 채도, 눈동자, 머리카락, 대비감 관찰
    - 8개 전통 키워드 중 2~3개 선정 + 100점 배분
    ↓
[3] 타입 확정
    - 1위 70점 이상 → 기본 타입 확정
    - 1위 70점 미만 → (1위, 2위) 조합으로 브릿지 타입 매칭
    ↓
[4] GPT-4.1-mini — ANSWER_PROMPT
    - 확정된 타입 정보를 기반으로 상세 답변 생성
    ↓
결과: 일러스트 카드 + 타입명 + 시즌 + 감성멘트 + 컬러 팔레트
```

**프론트 결과 UI:**
- 타입별 캐릭터 일러스트 (800×360, 가로 카드)
- 🎨 타입명(크게) + 🏷️ 시즌 태그(작게) + "감성 멘트"(이탤릭)
- 추천 컬러 팔레트 (HEX 기반 자동 생성) + 메이크업/헤어 추천

**컬러 팔레트 HEX 파싱:**
GPT가 매번 다른 컬러명을 생성하여 매핑 테이블 누락 문제가 발생했습니다. 이를 해결하기 위해 이중 처리 전략을 적용했습니다.

1. **프롬프트 강제**: ANSWER_PROMPT에 "모든 컬러를 `컬러명(#HEX)` 형식으로 출력" 규칙 추가
2. **프론트 폴백**: HEX가 없으면 기존 COLOR_NAME_TO_HEX 매핑 테이블(200+개)로 폴백 + 사용자 화면에서는 `stripHexCodes()`로 HEX 코드 숨김

---

#### 4.4.3. 웹캠 실시간 얼굴 감지

퍼스널컬러 분석 시 웹캠으로 촬영할 때, **TensorFlow.js + MediaPipe FaceDetector**를 사용하여 브라우저에서 실시간 얼굴 감지를 수행합니다. 서버 통신 없이 프론트에서만 처리됩니다.

**3단계 얼굴 검증:**

| 단계 | 기준 | 효과 |
| --- | --- | --- |
| Keypoints 유효성 | 6개 특징점 좌표가 모두 0이면 오탐 | 감지 실패 걸러냄 |
| 얼굴 크기 | 감지 영역이 화면의 20% 이상 | 작은 물체(에어팟 등) 차단 |
| 가이드 영역 완전 포함 | 얼굴 박스 전체가 가이드 원 안에 있어야 함 | 반만 걸친 상태에서 촬영 방지 |

**동작 흐름:** 가이드 원 안에 얼굴 완전 진입 → 테두리 노란색 → 3초 카운트다운 → 중간 이탈 시 리셋 → 완료 시 자동 촬영. 수동 촬영 버튼도 유지하여 모델 로드 실패 시에도 촬영 가능합니다.

---

#### 4.4.4. OCR 모델 전환 (Qwen2.5-VL → GPT-4o-mini Vision)

기존 Qwen2.5-VL 7B 모델은 GPU 필수 환경(약 15GB VRAM)이라 CPU 서버에서 가동 불가능했습니다. **4개 모델을 비교 평가**하여 GPT-4o-mini Vision으로 전환했습니다.

**정확도 비교 (실제 화장품 5장 평균)**

| 모델 | F1 Score | CPU 속도 | 비고 |
| --- | --- | --- | --- |
| PaddleOCR v5 | 30.2% | 1~3초 | 곡면 글자 깨짐 심각 |
| EasyOCR | 35.7% | 3~8초 | 곡면 왜곡 취약 |
| GOT-OCR2.0 CPU | 48.8% | 30초~2분 | 한국어 정확도 부족 |
| **GPT-4o-mini Vision** | **85.8%** | **4~10초** | **곡면/왜곡/한국어 모두 우수** |

**선택 이유:**
1. F1 Score 85.8%로 다른 모델 대비 압도적
2. 프롬프트로 "전성분 영역만 추출하라"고 지시 가능 (다른 모델은 전체 텍스트 후처리 필요)
3. 기존 OpenAI API 활용으로 추가 설치 불필요
4. CPU/GPU 환경 무관 (API 방식), 건당 약 4원

---

#### 4.4.5. MBTI 챗봇 연동

피부 MBTI 결과 페이지에서 "추천 챗봇 대화"를 클릭하면 챗봇으로 이동하여 **MBTI 맞춤 답변**을 생성합니다. 프론트에서는 짧은 메시지만 전송하고, 백엔드에서 DB의 MBTI 데이터를 LLM 프롬프트에 자동 주입합니다.

```
[프론트] MBTI 결과 → "추천 챗봇 대화" 클릭 → ChatPage 자동 메시지 전송
    ↓
[백엔드] context_builder: DB에서 MBTI 데이터 로드 (타입명, 루틴, 케어팁)
    ↓
[백엔드] generator.py: "피부 MBTI" 감지 시 프롬프트에 MBTI 전체 데이터 주입
    ↓
[LLM] MBTI 타입명으로 시작하는 맞춤 답변 생성
```

**설계 원칙:**
- 프론트 수정 최소화 — ChatPage.tsx 핵심 로직 미변경
- MBTI는 재미 요소 — 일반 대화에는 미반영, MBTI 결과에서 이동한 경우에만 적용
- 새로고침 재전송 방지 — `window.history.replaceState({}, "")` 한 줄로 해결

---

#### 4.4.6. 분석 실패 안내 메시지 통일

퍼스널컬러/빠른/정밀/성분 분석에서 잘못된 사진(만화, 동물, 풍경 등) 업로드 시, 기존에는 `"잠시 오류가 발생했어요"`라는 일반 에러만 표시되었습니다. `generator.py`에서 `vision_result["mode"] == "error"` 감지 시 **LLM 호출 없이 즉시 분석별 맞춤 안내 메시지를 반환**하도록 개선했습니다.

| 분석 유형 | 안내 내용 |
| --- | --- |
| 퍼스널컬러 | "실제 사람의 정면 얼굴 사진이 필요해요" + 자연광 촬영 팁 |
| 빠른 분석 | "정면 얼굴 사진 1장을 준비해주세요" + 세안 후 촬영 팁 |
| 정밀 분석 | "정면·좌측·우측 3장이 필요해요" + 머리카락 정리 팁 |
| 성분 분석 | "화장품 전성분표 사진을 올려주세요" + 수평 촬영 팁 |

각 메시지에는 🧴 사진 가이드 + 🔍 왜 필요한지 + 💡 촬영 팁 3가지 섹션이 포함됩니다.

---

#### 4.4.7. 로딩 UX — 단계별 진행 표시 + 피부 상식 모달

챗봇 답변 생성에 13~27초가 소요되어 사용자의 대기 경험을 개선하기 위해, **단계별 진행 표시**와 **피부 상식 캐러셀 모달**을 구현했습니다.

**A. 단계별 진행 표시 (SSE)**

`chat_router.py`의 `_run_ai`에 `step_callback` 파라미터를 추가하여 파이프라인 실행 중 단계별 메시지를 SSE로 실시간 전송합니다.

| 실행 시점 | 표시 메시지 |
| --- | --- |
| 파이프라인 시작 | 🔍 피부 고민을 분석하고 있어요... |
| 이미지 확인 | 📸 업로드된 이미지를 확인하고 있어요... |
| 피부 분석 | 🔬 AI가 피부 상태를 분석하고 있어요... |
| 성분 분석 | 🏷️ 전성분을 추출하고 있어요... |
| 퍼스널컬러 | 🎨 퍼스널컬러를 분석하고 있어요... |
| 일반 채팅 | 📚 23만 건의 피부 데이터에서 근거를 찾고 있어요... |

**B. 피부 상식 캐러셀 모달**

LLM 답변 생성 중(전체 응답 시간의 50~70%) 화면 중앙에 모달을 띄워 피부 상식을 제공합니다. 100개 상식 중 랜덤 20개를 선택하고, 좌우 화살표로 넘기면서 볼 수 있습니다. 답변 완료 시 모달이 자동으로 닫힙니다.

```
[사용자 메시지 전송]
🔍 "피부 고민을 분석하고 있어요..."                ← 단계별 진행 (채팅 말풍선)
📚 "23만 건의 데이터에서 근거를 찾고 있어요..."     ← 단계별 진행 (채팅 말풍선)
    ↓ 모달 자동 열림
┌──────────────────────────────────┐
│        💡 오늘의 피부 상식         │              ← 모달 (화면 중앙)
│            1 / 20                │
...
#### 4.4.7 FastAPI → Django 전환

Django의 URL routing + views 구조로 API 엔드포인트만 옮기고, 기존 services/db/ai 코드는 그대로 유지하는 **최소 변경 전략**을 채택했습니다. Django ORM은 사용하지 않고, 기존 pymysql + raw SQL 방식을 유지합니다.

**변경 사항**

| 항목 | FastAPI (기존) | Django (전환) |
| --- | --- | --- |
| 진입점 | `back/main.py` | `back/manage.py` |
| 라우팅 | `back/routers/*.py` (APIRouter) | `back/api/views/*.py` (Django views) |
| URL 설정 | 자동 (APIRouter prefix) | `back/config/urls.py` + `back/api/urls.py` |
| CORS | FastAPI `CORSMiddleware` | `django-cors-headers` |
| 인증 | `Depends(get_current_user_id)` | `api/middleware.py` (`require_auth` 데코레이터) |
| SSE 스트리밍 | FastAPI `StreamingResponse` | Django `StreamingHttpResponse` |
| DB 접근 | pymysql (변경 없음) | pymysql (변경 없음) |
| AI 파이프라인 | ai/ 폴더 (변경 없음) | ai/ 폴더 (변경 없음) |

34개 API 엔드포인트의 URL, 요청/응답 형식이 동일하게 유지되어 **프론트엔드 수정 없이** 전환 가능합니다.
...
### 4.8 Docker + Nginx
인프라 구조
이 프로젝트는 Docker Compose로 3개의 컨테이너를 묶어 운영합니다.
...
| backend | (Dockerfile 빌드) | FastAPI 앱 서버, AI 분석 처리 |
| chromadb | chromadb/chroma | AI 임베딩 벡터 저장소 |

---

nginx 라우팅 규칙
HTTP(80) → HTTPS(443) 자동 리다이렉트
/auth/ → 소셜 로그인 콜백, 백엔드 프록시
/users, /chats, /upload, /keywords, /analysis, /wishlist, /qna, /skin-mbti → 백엔드 프록시
그 외 모든 경로 → 프론트엔드 정적 파일 (try_files로 SPA 라우팅 지원)
업로드 파일 최대 크기: 20MB
SSL 인증서: Let's Encrypt (/etc/letsencrypt 마운트)
...
| 볼륨 경로 | 마운트 위치 | 설명 |
|---|---|---|
| ./vector_store | backend, chromadb 공유 | AI 벡터 데이터 |
| ./skin_ai | backend | 피부 AI 모듈 |
| ./ai | backend | AI 공통 모듈 |
| ./front/dist | nginx | 빌드된 프론트엔드 |

---

백엔드 이미지 빌드 (Dockerfile)
베이스: python:3.12-slim
시스템 라이브러리: MariaDB 드라이버, OpenCV용 그래픽 라이브러리 포함
PyTorch: CPU 전용 버전 설치 (용량 최적화)
...
주의: 최초 실행 시 PyTorch 설치로 인해 백엔드 이미지 빌드에 수 분이 소요될 수 있습니다.
...
본 프로젝트는 Flutter 기반의 AI 피부 분석/상담 챗봇 앱으로도 확장하였습니다.  
사용자는 사진 촬영, 이미지 업로드, 성분표 촬영, 자연어 질문 등을 통해 피부 상태를 분석하고, 맞춤형 제품 추천 및 상담 결과를 받을 수 있다. 또한 얼굴 이미지를 기반으로 퍼스널컬러를 진단하고, 어울리는 색상 가이드를 함께 제공한다.

### 1-1. 핵심 기능 요약

| 기능 | 설명 | 사용 모델/기술 |
|---|---|---|
| 빠른 분석 | 정면 사진 1장을 기반으로 수분/탄력/주름/모공/색소 5개 항목을 빠르게 분석 | ResNet50 (Fast Model) |
| 정밀 분석 | 정면/좌/우 3장 얼굴 사진을 기반으로 부위별 세부 피부 상태를 정밀 분석 | ResNet50 + AttentionFusion (Deep Model) |
| 성분 분석 | 화장품 전성분 라벨 사진을 촬영하거나 업로드하여 OCR 추출 후 피부 타입/고민 기반 성분 해석 제공 | Qwen2.5-VL 7B |
| 퍼스널컬러 진단 | 정면 얼굴 사진 1장을 기반으로 피부 톤과 분위기를 분석하여 퍼스널컬러 타입 및 추천 컬러 가이드를 제공 | Vision 기반 분석 + 챗봇 파이프라인 |
| 피부 상담 | 사용자의 자연어 질문에 대해 피부 상태, 루틴, 성분, 고민 중심 상담 제공 | GPT-4.1-mini + ChromaDB |
| 제품 추천 | RAG 후보 제품 중 실제 URL 검증을 거친 제품만 추천 | GPT-4.1-mini + Tavily |
| 얼굴 검증 | 업로드한 사진이 사람 얼굴인지, 정면/좌/우 순서가 적절한지 검증 | GPT-4.1-mini (Vision) |
| Intent 라우팅 | 사용자 입력을 회원/비회원 포함 여러 intent로 분류하여 적절한 기능으로 연결 | GPT-4.1-mini |
| 위시리스트 | 추천 제품 저장/조회/삭제 기능 제공 | 앱 상태관리 + DB 연동 |
| 피부 MBTI | 사용자의 응답을 기반으로 피부 성향 테스트 및 결과 제공 | 로컬 데이터 기반 로직 |

### 1-2. 기능별 앱 파이프라인

#### 1) 빠른 분석 파이프라인
1. 사용자가 `quick_face_capture_screen`에서 정면 사진 1장을 촬영하거나 업로드한다.
2. `quick_capture_review_screen`에서 이미지를 확인한다.
3. `analysis_request_service`가 이미지를 정리하여 API 요청을 생성한다.
4. `api_service`가 백엔드로 이미지를 업로드하고 분석 요청을 전송한다.
5. 분석 결과를 `chatbot_screen` 또는 결과 화면에서 사용자에게 제공한다.

#### 2) 정밀 분석 파이프라인
1. 사용자가 `face_capture_screen`에서 정면/좌/우 총 3장의 사진을 촬영하거나 업로드한다.
2. `capture_review_screen`에서 3장의 이미지를 검토한다.
3. 얼굴 검증 및 순서 검증을 수행한 뒤 분석 요청을 생성한다.
4. 백엔드의 정밀 분석 모델이 부위별 피부 상태를 분석한다.
5. 결과를 챗봇 대화 흐름 또는 피부 분석 결과 페이지로 전달한다.

#### 3) 성분 분석 파이프라인
1. 사용자가 `ingredient_capture_screen`에서 화장품 성분표 이미지를 촬영하거나 업로드한다.
2. OCR 및 비전 모델을 통해 전성분 텍스트를 추출한다.
3. 추출된 성분을 피부 타입/피부 고민 기준으로 해석한다.
4. 챗봇 화면에서 성분별 특징과 주의 포인트를 설명한다.

#### 4) 피부 상담 / 제품 추천 파이프라인
1. 사용자가 `chatbot_screen`에서 자연어 질문을 입력한다.
2. Intent 분류를 통해 질문 유형을 판별한다.
3. RAG 검색으로 관련 지식, 루틴, 제품 정보를 조회한다.
4. 제품 추천이 필요한 경우 실사용 가능한 URL을 검증한다.
5. 검증된 정보만 사용자에게 응답으로 제공한다.

#### 5) 피부 MBTI 파이프라인
1. 사용자가 `skin_mbti_test_screen`에서 문항에 응답한다.
2. 로컬 데이터(`skin_mbti_data.dart`)를 기준으로 결과를 계산한다.
3. `skin_mbti_result_screen`에서 피부 성향 결과와 설명을 제공한다.

#### 6) 퍼스널컬러 진단 파이프라인
1. 사용자가 퍼스널컬러 진단 화면에서 정면 사진 1장을 촬영하거나 업로드한다.
2. 촬영 또는 업로드한 이미지를 검토한 뒤 분석 요청을 생성한다.
3. `api_service`가 이미지를 백엔드 또는 챗봇 분석 파이프라인으로 전달한다.
4. 얼굴 이미지 기반 톤 분석을 통해 퍼스널컬러 타입을 추정한다.
5. 진단 결과와 함께 어울리는 컬러, 피해야 할 컬러, 스타일 가이드를 사용자에게 제공한다.

### 1-3. 앱 내 주요 사용자 기능

- 회원가입 / 로그인 / OAuth 로그인
- 홈 화면에서 빠른 분석, 정밀 분석, 성분 분석, 퍼스널컬러 진단, 피부 상담 진입
- 촬영 또는 이미지 업로드 가이드 제공
- 분석 결과 확인 및 챗봇 연계
- 제품 위시리스트 저장
- FAQ / 문의 / 회원탈퇴 / 설정 화면 제공
