...
수준까지 확대될 수 있습니다. 즉, 최근의 실제 성장 흐름을 그대로 단순 연장하면, 향후 3년 내 올리브영 중심 K-뷰티 플랫폼 시장은 **약 10조~13조 원 규모**로 커질 가능성이 있다고 볼 수 있습니다. 

온유는 이처럼 빠르게 성장하는 뷰티 커머스 환경에서 피부 분석, 성분 해석, 제품 추천, 퍼스널컬러, 피부 MBTI를 하나의 흐름으로 연결해 **개인화된 뷰티 경험**을 제공하는 AI 서비스로 기획되었습니다. 특히 분석 결과를 저장하고 다시 확인하며, 필요 시 공유까지 이어지는 구조를 통해 단순 체험형 기능이 아니라 **재방문 가능한 서비스 경험**을 목표로 합니다.

### 2.5. 기대효과
- 피부 분석, 성분 해석, 퍼스널컬러, 피부 MBTI를 하나의 사용자 흐름으로 통합
...
- 색조/스타일링 중심 사용자 경험이 잘 정리되어 있음 

#### 한계점
- 서비스 중심축이 퍼스널컬러에 맞춰져 있어, 피부 정량 분석이나 성분 해석까지 한 번에 연결되는 구조는 상대적으로 약함
- 스킨케어 상담, 피부 분석 이력 관리, 결과 공유형 리포트 등 확장형 흐름보다는 컬러 진단 중심 경험에 강점이 있음 
...
- 포인트, 이벤트 등 앱 서비스형 요소 제공 

#### 강점
- 퍼스널컬러와 피부 진단을 함께 제공해 뷰티 앱으로서의 범용성이 높음
...
- 피부 상담형 챗봇, 공유형 결과 링크, 분석→성분→추천을 하나로 묶는 구조는 온유가 더 직접적으로 설계한 영역에 가깝다 할 수 있음 

---

### 3.3. 온유의 차별점

온유는 퍼스널컬러나 피부 진단 중 한 영역에만 집중하기보다, **피부 분석 → 성분 해석 → 제품 추천 → 저장/공유 → 재방문**까지 이어지는 통합 흐름을 설계한 것이 가장 큰 차별점입니다. 아이컬러가 퍼스널컬러 특화 경험에 강점이 있고, 잼페이스가 진단과 콘텐츠 소비를 함께 제공하는 앱형 경험에 강점이 있다면, 온유는 여기에 **성분 OCR, 챗봇 기반 상담, 날짜별 분석 이력, 피부 MBTI, 공유 기능**까지 결합해 보다 연결된 사용자 경험을 제공하는 방향을 지향합니다.
...
기존 3차 버전은 피부 분석, 성분 분석, 제품 추천 챗봇 중심의 구조였다면, 4차 버전에서는 피부 MBTI, 분석 이력, 공유, 온보딩, 정책 페이지, 마이페이지 기능까지 확장해 서비스 완성도를 높였습니다. 기존 문서상 앞단 기능은 피부 MBTI, 공유, 회원가입 UX, 마이페이지 기능 강화로 정리되어 있었습니다. 
...
| 3 | 웹캠 얼굴 감지 | 🛡️ 입력 품질 | 아무 사진 업로드 가능 | 실시간 3단계 검증 + 자동 촬영 |
...
**컬러 팔레트 HEX 파싱:**
GPT가 매번 다른 컬러명을 생성하여 매핑 테이블 누락 문제가 발생했습니다. 이를 해결하기 위해 이중 방어 전략을 적용했습니다.
...
#### 4.4.3. OCR 모델 전환 (Qwen2.5-VL → GPT-4o-mini Vision)
...
#### 4.4.6. 로딩 UX — 단계별 진행 표시 + 피부 상식 모달
